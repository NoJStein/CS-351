...
Type* loadData(const char* path) {
    int fd = open(path, O_RDONLY);	// Opens a file descriptor
    off_t size = lseek(fd, 0, SEEK_END);	// Finds the file's length in bytes

    void* memory = mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0); // Maps all data into memory, returning a void
								    // pointer to the beginning of the region.
    
    return reinterpret_cast<Type*>(memory);	// Mapped data to be seen as an array of Type
}

int main(int argc, char* argv[]) {
    using Byte = unsigned char;
    using Bytes = Byte*;
    Bytes data = loadData<Byte>(path);	// data points to the first byte of Data.bin in memory
